...
func NewMssqlProvisioner(logger lager.Logger) MssqlProvisionerInterface {
...
func (provisioner *MssqlProvisioner) Connect(goSqlDriver string, connectionParams map[string]string) error {
	var err error = nil
	connString := buildConnectionString(connectionParams)

	provisioner.dbClient, err = sql.Open(goSqlDriver, connString)
	if err != nil {
		return err
	}
